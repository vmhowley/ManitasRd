...
export const ServiceCard: React.FC<ServiceCardProps> = ({
      ...
      >
        <div className="flex items-center p-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-medium text-sm text-neutral-800 truncate">{service.name}</h3>
            <p className="text-xs text-neutral-500 truncate">{service.category}</p>
          </div>
          <div className="text-right">
            <p className="font-medium text-sm text-primary-600">RD$ {service.price.toLocaleString()}</p>
          </div>
        </div>
      </Card>
    );
  ...
  if (variant === 'featured') {
    return (
      <Card className="overflow-hidden rounded-2xl h-36 relative cursor-pointer" onClick={onSelect}>
        <img 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex flex-col justify-end p-4">
          {service.rating && (
            <Badge variant="primary" className="mb-1 w-fit">{service.rating.toFixed(1)} ★</Badge>
          )}
  ...
  return (
    <Card 
      className="group flex flex-col h-full transition-all duration-300 hover:translate-y-[-4px] rounded-xl overflow-hidden shadow-md hover:shadow-lg"
      onClick={onSelect}
    >
      ...
      <CardContent className="p-4 flex-grow flex flex-col">
        ...
        {service.description && (
          <p className="text-sm text-neutral-700 mb-3 line-clamp-2">{service.description}</p>
        )}
